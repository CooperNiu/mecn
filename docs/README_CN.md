# MECN 文档索引

**语言**: [中文](README_CN.md) | [English](README_EN.md)

---

## 📚 快速导航

### 入门指南

| 文档 | 描述 | 语言 |
|------|------|------|
| [README](../README.md) | 项目概览和快速开始 | 🇨🇳 中文 |
| [README_EN](../README_EN.md) | Project overview and quick start | 🇺🇸 English |
| [CLI使用指南](user/CLI_USAGE_GUIDE.md) | 命令行工具完整使用文档 | 🇨🇳 中文 |
| [API使用指南](user/API_USAGE.md) | RESTful API接口文档 | 🇨🇳 中文 |

### 开发文档

| 文档 | 描述 | 语言 |
|------|------|------|
| [模块结构说明](dev/MODULE_STRUCTURE.md) | 模块架构详细说明 | 🇨🇳 中文 |
| [Module Structure (EN)](dev/MODULE_STRUCTURE_EN.md) | Module architecture details | 🇺🇸 English |
| [TDD实践记录](dev/TDD_PRACTICE_RECORD.md) | 测试驱动开发实践记录 | 🇨🇳 中文 |
| [TDD Practice Record (EN)](dev/TDD_PRACTICE_RECORD_EN.md) | TDD development records | 🇺🇸 English |
| [测试指南](dev/TESTING.md) | 测试实践和指南 | 🇨🇳 中文 |
| [Testing Guide (EN)](dev/TESTING_EN.md) | Testing practices and guidelines | 🇺🇸 English |

### 部署与运维

| 文档 | 描述 | 语言 |
|------|------|------|
| [部署指南](ops/DEPLOYMENT.md) | 部署说明和配置 | 🇨🇳 中文 |
| [Deployment Guide (EN)](ops/DEPLOYMENT_EN.md) | Docker deployment guide | 🇺🇸 English |
| [CI/CD指南](ops/CICD_GUIDE.md) | 持续集成/持续部署 | 🇨🇳 中文 |
| [CI/CD Guide (EN)](ops/CICD_GUIDE_EN.md) | CI/CD configuration guide | 🇺🇸 English |
| [MkDocs指南](ops/MKDOCS_GUIDE.md) | 文档站点构建指南 | 🇨🇳 中文 |
| [MkDocs Guide (EN)](ops/MKDOCS_GUIDE_EN.md) | MkDocs documentation site guide | 🇺🇸 English |

### 项目管理

| 文档 | 描述 | 语言 |
|------|------|------|
| [项目进展报告](pm/PROJECT_PROGRESS_REPORT.md) | 当前开发状态和进度 | 🇨🇳 中文 |
| [阶段一完成总结](pm/PHASE1_COMPLETION_SUMMARY.md) | 阶段一完成情况总结 | 🇨🇳 中文 |
| [阶段一执行总结](pm/PHASE1_EXECUTION_SUMMARY.md) | 阶段一执行情况总结 | 🇨🇳 中文 |
| [工作计划](pm/workPlan.md) | 开发工作计划 | 🇨🇳 中文 |

---

## 🎯 推荐阅读路径

### 新用户

1. **[README](../README.md)** - 了解项目功能特性
2. **[CLI使用指南](CLI_USAGE_GUIDE.md)** - 学习使用命令行工具进行分析
3. **[API使用指南](API_USAGE.md)** - 将MECN集成到您的应用中

### 开发者

1. **[模块结构说明](MODULE_STRUCTURE.md)** - 理解代码组织和架构
2. **[TDD实践记录](TDD_PRACTICE_RECORD.md)** - 学习项目的TDD开发实践
3. **[测试指南](TESTING.md)** - 运行测试和编写新测试
4. **[项目进展报告](PROJECT_PROGRESS_REPORT.md)** - 了解当前开发状态和计划

### 运维人员

1. **[部署指南](DEPLOYMENT.md)** - 部署应用到服务器或云平台
2. **[CI/CD指南](CICD_GUIDE.md)** - 理解持续集成/部署流程
3. **[MkDocs指南](MKDOCS_GUIDE.md)** - 构建和维护文档站点

---

## 📖 文档分类

### 用户文档

- **快速入门指南** - 上手教程和示例
- **用户手册** - 功能使用说明
- **常见问题(FAQ)** - 常见问题解答
- **示例代码** - 代码示例和演示

### 开发文档

- **架构概览** - 系统架构设计
- **API参考** - 详细的API文档
- **开发指南** - 编码规范和最佳实践
- **测试策略** - 测试方法和工具

### 运维文档

- **部署指南** - 安装和部署说明
- **配置参考** - 配置选项详解
- **监控与日志** - 系统监控方法
- **故障排除** - 常见问题和解决方案

---

## 🔗 外部资源

- **GitHub仓库**: [https://github.com/CooperNiu/mecn](https://github.com/CooperNiu/mecn)
- **问题追踪**: [https://github.com/CooperNiu/mecn/issues](https://github.com/CooperNiu/mecn/issues)
- **讨论区**: [https://github.com/CooperNiu/mecn/discussions](https://github.com/CooperNiu/mecn/discussions)

---

## 📝 贡献文档

我们欢迎文档改进！如果您发现任何问题或有建议：

1. 检查[Issues](https://github.com/CooperNiu/mecn/issues)中是否已存在相关问题
2. 创建新的Issue描述文档问题
3. 提交Pull Request包含您的改进

**文档标准**:
- 使用清晰简洁的语言
- 在适当位置包含代码示例
- 保持格式一致性
- 修改时更新相关文档

---

## 📊 文档统计

- **文档总数**: 17个
- **中文文档**: 16个
- **英文文档**: 1个 (README_EN.md)
- **最后更新**: 2026-04-17

---

## 🌐 国际化计划

**当前状态**: 
- ✅ 核心README提供中英文版本
- ⏸️ 技术文档主要为中文
- 📋 计划翻译关键文档为英文

**翻译优先级**:
1. CLI使用指南
2. API使用指南  
3. 部署指南
4. 测试指南

**想帮忙？** 我们欢迎志愿者帮助翻译文档！请通过GitHub Issues联系我们。

---

**维护者**: MECN团队  
**最后审查**: 2026-04-17  
**版本**: v1.5.0-dev
